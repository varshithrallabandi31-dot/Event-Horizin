    ...
    <?php else: ?>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <?php foreach($events as $event): ?>
                <a href="<?php echo BASE_URL; ?>event/<?php echo $event['id']; ?>" class="group bg-white rounded-3xl overflow-hidden border-2 border-charcoal-200 hover:border-brand-400 transition-all duration-300 hover:shadow-xl hover:shadow-brand-500/10 hover:-translate-y-1">
                    <div class="relative h-48 overflow-hidden">
                        ...
                        <div class="absolute top-3 left-3 px-3 py-1 bg-white/90 backdrop-blur-md rounded-lg text-[10px] font-bold text-charcoal-900 uppercase tracking-wider border border-white/20 shadow-sm">
                            <?php echo htmlspecialchars($event['category']); ?>
                        </div>
                    </div>
                    <div class="p-5">
                        <h3 class="text-lg font-bold text-charcoal-900 mb-2 line-clamp-1 group-hover:text-brand-600 transition"><?php echo htmlspecialchars($event['title']); ?></h3>
                        <div class="space-y-2">
                            <p class="flex items-center gap-2 text-xs text-brand-600 font-bold">
                                <i data-lucide="calendar" class="w-3.5 h-3.5"></i>
                                <?php echo date('M d, D', strtotime($event['start_time'])); ?> • <?php echo date('g:i A', strtotime($event['start_time'])); ?>
                            </p>
                            <p class="flex items-center gap-2 text-xs text-charcoal-500 truncate">
